from django.urls import path
from . import views

urlpatterns = [
    path('', views.CargaListView.as_view(), name='carga_list'),
    path('nueva/', views.carga_layout, name='carga_new'),
    path('<int:pk>/', views.carga_detalle, name='carga_detalle'),
    path('<int:pk>/comprobante/', views.carga_comprobante, name='carga_comprobante'),
    path('<int:pk>/revalidar/', views.carga_revalidar, name='carga_revalidar'),
    path('<int:pk>/aceptar/', views.carga_aceptar, name='carga_aceptar'),
    path('<int:pk>/rechazar/', views.carga_rechazar, name='carga_rechazar'),
    path('<int:pk>/fila/<int:fila>/decidir/', views.carga_decidir_fila, name='carga_decidir_fila'),
    path('<int:pk>/rechazar-pendientes/', views.carga_rechazar_pendientes, name='carga_rechazar_pendientes'),
    path('calendario/', views.calendario_cargas, name='calendario_cargas'),
    path('periodos/', views.PeriodoCargaListView.as_view(), name='periodo_list'),
    path('periodos/nuevo/', views.PeriodoCargaCreateView.as_view(), name='periodo_create'),
    path('periodos/generar/', views.generar_periodos, name='periodo_generar'),
    path('periodos/<int:pk>/activar/', views.activar_periodo, name='periodo_activar'),
    path('excepciones/', views.AccesoExcepcionListView.as_view(), name='excepcion_list'),
    path('excepciones/nueva/', views.AccesoExcepcionCreateView.as_view(), name='excepcion_create'),
    path('excepciones/<int:pk>/eliminar/', views.AccesoExcepcionDeleteView.as_view(), name='excepcion_delete'),
    path('descargar-plantilla/<str:tipo>/', views.descargar_plantilla, name='descargar_plantilla'),
]
